...
import { BrainCircuit, TrendingUp, CheckSquare, Zap, Mail, CreditCard } from 'lucide-react';
...
export default function FreePage() {
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            ...
            <div className="flex flex-col md:flex-row gap-6">
              <div className="flex-1">
                <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-4" style={{ background: 'rgba(99, 102, 241, 0.1)' }}>
                  <resource.Icon className="w-6 h-6 text-indigo-400" aria-hidden="true" />
                </div>
                <h2 className="text-xl font-bold text-white mb-2">{resource.title}</h2>
                <p className="text-zinc-500 mb-6 text-sm leading-relaxed">{resource.description}</p>
                <div className="flex items-center gap-4 text-sm text-zinc-600">
                  <span className="flex items-center gap-1.5">
                    <Zap className="w-3.5 h-3.5 text-indigo-400/60" aria-hidden="true" />
                    Instant delivery
                  </span>
                  <span className="flex items-center gap-1.5">
                    <Mail className="w-3.5 h-3.5 text-indigo-400/60" aria-hidden="true" />
                    To your inbox
                  </span>
                  <span className="flex items-center gap-1.5">
                    <CreditCard className="w-3.5 h-3.5 text-indigo-400/60" aria-hidden="true" />
                    No card needed
                  </span>
                </div>
              </div>
      ...
      {/* CTA */}
      <div className="mt-16 glass rounded-2xl p-10 text-center relative overflow-hidden" style={{ background: 'linear-gradient(135deg, rgba(99, 102, 241, 0.1), rgba(139, 92, 246, 0.05))' }}>
