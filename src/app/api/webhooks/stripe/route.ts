...
async function handleCheckoutCompleted(session: Record<string, unknown>) {
  ...
  // Send purchase confirmation email (best-effort, never block webhook)
  try {
    ...
    await sendEmail({
      to: customerEmail,
      subject: `Your AI Armory order is confirmed (#${orderId.slice(0, 8).toUpperCase()})`,
      html: purchaseConfirmation({
        customerEmail,
        productName,
    ...
    console.error(
      "[Webhook] Failed to send purchase confirmation email:",
      emailErr,
    );
  }
}
...
async function handleChargeRefunded(charge: Record<string, unknown>) {
  ...
  if (!order) {
    console.warn(
      `[Webhook] charge.refunded — no order found for payment_intent ${paymentIntent}`,
    );
    return;
  }
  ...
  console.log(
    `[Webhook] Refunded order ${order.id} for ${order.customer_email}`,
  );
}
